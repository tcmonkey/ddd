...
> 本版依据当前源码整理实现基线，将已被覆盖的历史选择移到归档。它不是本轮重新编码方案；本次 Java、POM、配置和 Checkstyle 均不改动。原技术审批记录保留，新整理版供阅读反馈。
...
// Application：当前 @Transactional，含组装与调用的边界异常处理。
Result<DddWriteResult> write(DddWriteCommand dddWriteCommand) {
    ...
    DddWriteResult result = assembler.toResult(dddWriteCommand, domainResult.data());
...
Result<DddWriteDO> write(DddWriteParam param) {
    ...
    DddRuleAggregate rule = ruleRepository.getRequiredByRuleCode(pending.ruleCode());
...
查询不存在的主对象返回可首次写入空聚合；规则不存在明确抛 DomainException。主保存区分 insert/带旧版本 updateById；乐观锁能力由 start 的 MyBatis-Plus 配置装配。
...
- 主边界捕获已知异常返回友好码；未知异常记录堆栈并转本层 PROCESS_FAILED，不向外透出技术细节。当前 domain 对 InfrastructureException 按未知异常统一映射为 DOMAIN_PROCESS_FAILED。
...
| CR | 对实际源码/基线做架构、错误、事务、并发、转换和生产适配审查 | 本轮仅文档校准，不标 complete |
...
| TASK-004 | NF003/006、TECH-025 | Java 规范、Checkstyle、门禁安装器和快照 | TASK-001～003 | 自动/部分/评审边界，违规编译前拒绝 | 已实现，5 项机制回归 |
| TASK-005 | F014、TECH-028 | solo 阶段/资产/脚本、ddd AI/output | 用户 SRC-039 | 按需初始化、迁移、阶段隔离与文档一致 | 本轮实施 |
...
重点待审查事项：Result 失败与 @Transactional 的回滚语义；首次并发写/乐观锁冲突；同一幂等 ID 不同载荷；私有协议异常到友好错误的映射。这些是主动识别的审查候选，本轮未完整验证或改动，不把它们写成已关闭发现。

## 5. 工时与人天评估

本轮是既有工程的流程/文档重整，不重复估算历史编码，也不把 AI 工具运行时长当作实际人天。

| 工作包 | 后续人工估算参考 | 条件/不确定性 |
|---|---:|---|
| 阅读确认本轮文档与规则 | 0.25～0.5 人天 | 已熟悉工程，主要核对语义与裁剪 |
| 全量 CR 与修复复查 | 0.5～1.5 人天 | 随发现数量变化；未在本轮执行 |
| 独立用例、功能/边界/回归 | 0.5～1.5 人天 | 发布范围先明确，不含未定义性能专项 |
| 生产适配、发布与效果 | 暂不估算 | 无目标环境、指标、真实依赖与授权 |
| 已定义后续合计 | 1.25～3.5 人天 | 估算非承诺，也非已经投入的人天 |

变更：3.0 以当前代码重整 TECH-001～027，新增 TECH-028 文档/阶段闭环；将计划内嵌、验证和发布回退归到整体方案。完整旧方案及每轮变更见工作台归档链接。
