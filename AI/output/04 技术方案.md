...
| TECH-014 | 公共参考工程的仓储链路应最小且完整。 | 只保留一张 `ddd_data` 表、一个 `DddPO`、一个 `DddMapper` 和一个领域仓储实现；领域实体仍由聚合持有，但操作记录以 `entities_json` 同行保存，用于重建历史幂等标识。 |
...
| DATA-006 | `ddd_data` | 唯一的 H2 示例表；同一行保存聚合主状态、版本和 `entities_json` 实体快照。 |
...
DddWriteResult execute(DddWriteCommand command) {
    ...
    aggregate.write(decision.operationId(), decision.value(),
        command.ruleCode(), clock.instant());
...
Boolean save(DddAggregate aggregate) {
    ...
    return super.updateById(snapshot); // MyBatis-Plus @Version 校验并推进版本。
}
```
...
DddReadResult query(String id) {
    ...
    return toView(aggregate); // 聚合内实体转换为应用层读结果。
}
...
- 唯一的 `DddMapper` 继承 `DddBaseMapper<DddPO> → BaseMapper<DddPO>`。
- `DddBaseRepository<M, T>` 只继承 `CrudRepository<M, T>`；父类的 `baseMapper` 已由 Spring `@Autowired`，不自定义初始化构造器。
- 唯一的 `DddRepositoryImpl` 实现 domain `DddRepository`，继承基础仓储 CRUD；负责 `DddPO` 与聚合之间的重建、实体 JSON 快照读写及乐观锁判断。
