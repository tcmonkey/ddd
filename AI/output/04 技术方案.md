...
| 域内读 | `GET /api/ddd/{id}` | input → `DddReadCommand` → application assembler → `DddReadParam` → `DddRepository` | repository 直接返回聚合根，由 application assembler 转为结果。 |
...
Result<DddWriteDO> execute(DddWriteParam param) {
  try {
    ...
    DddReadParam readParam = new DddReadParam(input.id().value());
...
Result<DddReadResult> query(DddReadCommand command) {
    try {
        DddReadParam param = applicationAssembler.toDomainParam(command);
