...
| TECH-003 | application 负责编排，不承载领域状态变化。 | application 类使用 Spring `@Service`，容器依赖统一使用字段 `@Autowired`；写逻辑委托 domain service/aggregate。 |
...
| TECH-006 | 类和注释需要形成可复制规范。 | aggregate/entity/param/value 使用明确后缀；源码注释使用多行中文 Javadoc，每个注释含 `@author`；本模板作者统一为 `AIGenerator`。 |
...
| TECH-012 | Spring 组件注入方式需统一。 | Controller、application、output adaptor 使用字段 `@Autowired`；仓储的 `ObjectMapper` 同样使用字段 `@Autowired`，Mapper 注入复用 MyBatis-Plus `CrudRepository` 的已有标记。domain 无 Spring 依赖，领域服务通过 start 装配。 |
...
- `ddd-client`：公开请求、响应和 `Result<T>`，不包含业务实现；只有真实提供 RPC 能力时才增加带完整 Javadoc 契约的二方包接口。
...
| DATA-006 | `ddd_data` | 主聚合唯一的 H2 示例表；同一行保存聚合主状态、版本和 `entities_json` 实体快照。 |
| DATA-007 | `ddd_rule` | 规则聚合独立空表；保存 `rule_code`、`factor`、`reason`，运行期不预置样本，测试中按需插入。 |
...
Result<DddWriteResponse> write(DddWriteRequest request) {
    ...
    return Result.success(inputAssembler.toResponse(result));
}
...
DddWriteResult execute(DddWriteCommand command) {
    ...
    DddWriteParam param = new DddWriteParam(
        new DddOperationIdValue(command.operationId()),
        command.ruleCode(), DddValue.positive(command.baseValue()));
    DddDecisionParam decision = dddWriteDomainService.decide(aggregate, param);
    if (decision.duplicate()) {
        return new DddWriteResult(id.value(), param.operationId().value(),
            decision.value().value(), aggregate.currentValue().value(), true);
    }
    aggregate.write(decision.operationId(), decision.value(),
        command.ruleCode(), Instant.now());
    if (!Boolean.TRUE.equals(dddRepository.save(aggregate))) {
        throw new ConcurrentModificationException("保存失败或版本冲突");
    }
    return new DddWriteResult(id.value(), param.operationId().value(),
        decision.value().value(), aggregate.currentValue().value(), false);
}

DddAggregate findById(DddIdValue id) {
    DddPO stored = getById(id.value());
    if (stored == null) return DddAggregate.open(id);
    return DddAggregate.restore(id, new DddValue(stored.getCurrentValue()),
        stored.getVersion(), readEntities(stored.getEntitiesJson()));
}
...
Boolean save(DddAggregate aggregate) {
    ...
    DddPO snapshot = toDddPO(aggregate, aggregate.version() - 1);
    return super.updateById(snapshot);
}
```

更新路径通过 MyBatis-Plus `@Version` 校验并推进版本；实体列表在 `toDddPO` 中序列化为主表 JSON 快照。

域内读、规则计算和纯计算：

```java
DddReadResult query(String id) {
    DddAggregate aggregate = dddRepository.findById(new DddIdValue(id));
    return toView(aggregate);
}

DddRuleResult execute(DddRuleCommand command) {
    DddRuleAggregate rule = dddRuleRepository.getRequiredByRuleCode(command.ruleCode());
    DddRuleParam param = new DddRuleParam(
        command.ruleCode(), DddValue.positive(command.baseValue()));
    DddValue calculated = rule.evaluate(param);
    return new DddRuleResult(rule.ruleCode(), rule.factor(),
        calculated.value(), rule.reason());
}

DddRuleAggregate getRequiredByRuleCode(String ruleCode) {
    DddRulePO stored = getById(ruleCode);
    if (stored == null) throw new DomainValidationException("规则不存在");
    return new DddRuleAggregate(stored.getRuleCode(), stored.getFactor(), stored.getReason());
}

DddCalculateResult execute(DddCalculateCommand command) {
    DddValue value = calculateDomainService.calculate(
...
```java
DddExternalResult query(String id) {
    DddModel model = outputAdaptor.queryById(id);
    return new DddExternalResult(model.id(), model.name(), model.category());
}

DddModel queryById(String id) {
    DddExternalResponse remote = new DddExternalResponse(
        id, "DDD_EXTERNAL_" + id, "DEFAULT");
    return converter.toModel(remote);
}
```
...
- `DddRepositoryImpl` 实现 domain `DddRepository`，继承基础仓储 CRUD；负责 `DddPO` 与主聚合之间的重建、实体 JSON 快照读写及乐观锁判断。
...
- `DddPO` 对应单表 `ddd_data`：`id`、`current_value`、`version`、`entities_json`；JSON 只用于演示聚合内实体，真实项目需依据查询、索引、容量和迁移要求决定是否拆表。
...
- Controller、application、output adaptor 使用字段 `@Autowired` 注入容器依赖；application 使用 `@Service`，output 实现使用 `@Component`。
- infrastructure 仓储使用 `@Repository`；`DddRepositoryImpl` 的 `ObjectMapper` 使用字段 `@Autowired`，Mapper 注入沿用 MyBatis-Plus 父类已有的 `@Autowired`，不重复声明构造器。
...
| 日志 | 模板的 `logback-spring.xml` 默认仅控制台输出；`prod` 额外输出按日期与 50MB 大小滚动的文件，保留 14 天、总量上限 2GB，默认目录为 `logs/`。可通过 `LOG_DIR` 和 `ROOT_LOG_LEVEL` 调整。日志格式预留 MDC `traceId` 字段，未设置时显示 `none`；当前尚未实现请求 traceId 填充或用例边界业务日志。真实项目应在边界补 traceId、调用模式、业务 ID、结果、耗时和异常类型，禁止记录完整敏感请求。 |
