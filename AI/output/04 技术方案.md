...
version: "2.5"
mode: "full"
updated: "2026-09-16T00:30:00+08:00"
...
| TECH-021 | 内部数据对象和跨层入参必须有稳定且可验证的边界。 | `XXPO` 仅在 infrastructure 映射数据库；`XXDO` 仅在 `ddd-model` 表示无行为内部数据；DomainService、OutAdaptor 公开返回 `Result<XXDO>`。跨 Application、DomainService、OutAdaptor 的任意入参一律为 `XXCommand` 或 `XXParam`，禁止裸基本类型；OutAdaptor 接收 Application Command 后由 converter 转为 adaptor 私有第三方请求。 |
...
Result<DddWriteDO> execute(DddWriteParam param) {
  try {
    ...
    DddAggregate aggregate = dddRepository.findById(input.id());
...
Result<DddReadResult> query(DddReadCommand command) {
    try {
        DddAggregate aggregate = dddRepository.findById(DddAggregate.idOf(command.id()));
...
Result<DddExternalResult> query(DddExternalReadCommand command) {
    ...
    return toApplicationResult(adaptorResult);
}
