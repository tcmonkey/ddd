...
DddWriteResult execute(DddWriteCommand command) {
    ...
    DddDecisionParam decision = dddWriteDomainService.execute(new DddWriteParam(input));
    return new DddWriteResult(command.id(), decision.operationId().value(),
        decision.value().value(), decision.currentValue().value(), decision.duplicate());
}

DddDecisionParam execute(DddWriteParam param) {
    ...
    if (existing != null) return duplicate(existing, entity.currentValue());
    ...
    if (!Boolean.TRUE.equals(dddRepository.save(aggregate))) throw new ConcurrentModificationException();
    return written(confirmed, entity.currentValue(), rule.reason());
}
