...
mode: "full"
updated: "2026-09-19"
sources: [SRC-001, SRC-002, SRC-005, SRC-007, SRC-035, SRC-037, SRC-038, SRC-039, SRC-040, SRC-041, SRC-042, SRC-043, SRC-044, SRC-045, SRC-049, SRC-050, SRC-051, SRC-052]
---
...
## 10. 3.7按业务采用开发模式（当前解释）

五种处理模式是同一业务领域可按需采用的用例路径：写入、域内普通读、外部普通读、规则计算、纯计算。先划业务，再在业务内选择模式；DddWriteDomainService.write表达ddd占位业务内写模式，不代表全项目所有写操作必须进入一个服务。真实项目用本业务领域/动作命名服务，Param/assembler/仓储协作归本业务；跨域应用明确协调，不生成模式总路由。普通读/外部读不为凑服务数建DomainService。

当前参考工程只有一个占位业务ddd，已有五条路径及三个领域服务都在domain/ddd内，不存在跨业务汇总，故不把它机械分拆为五业务。`DddWriteDomainService`保留，是为了标记本参考工程的教学路径；真实业务服务以业务职责命名，不拿`Write`等模式词充当领域身份。运行逻辑、签名、事务与DB配置不改。规范1.14/DDD-009～012、快照1.12明确示例边界，实际hello-travel的业务垂直整改证据留在该项目。DDD根和临时独立快照全部模块各7项通过，CR/生产验收和发布仍未执行。

## 11. 3.8技术能力归属与模型抽象规则

DDD-014补足“先业务、后技术端口”的下一层判断：HTTP、邮件、文件、安全、模型和向量不是与业务域平级的产品模块，先从用户任务确定所有者，再放入所属业务的input/output边界。问答、意图、摘要、事实提取和嵌入具有不同输入、结果、失败与流式契约，不通过一个动态action模型端口混合。

Application只依赖用途明确的业务语义端口；框架高阶接口与流式处理归adaptor，命名模型、工具、检索器和降级条件归start。采用AiService或自动RAG时仍须保留租户、删除代次、来源、审计和流式能力。当前ddd参考工程没有AI/RAG业务，不为展示该规则增加无用途模型类，既有五条教学链路和运行行为保持不变。

## 12. 3.9输入Controller与Assembler同级分包

每个业务的输入适配层使用同级`input.controller`和`input.assembler`子包，分别承载协议入口与双向协议转换；禁止把两类文件直接平铺在`input`根包。参考工程的DddController已迁入controller，DddInputAssembler保留在assembler，HTTP路径、调用链和转换职责不变。新增QUALITY-PACKAGE有限门禁，在编译前拒绝以类后缀可识别的错包结构。
