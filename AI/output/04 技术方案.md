...
| TECH-004 | 仓储查询直接返回聚合根，保存返回 `Boolean`。 | domain 只声明端口；ID/IDs 查询可用基础标识类型/集合，写入接收 Aggregate；infrastructure 使用 MyBatis-Plus 实现。 |
...
| TECH-009 | 版本管理必须集中。 | 根 POM 管理工程版本、Spring Boot parent、MyBatis-Plus BOM、`ddd-*` 模块依赖与插件版本；子模块 dependency/plugin 不声明版本。Maven 3 子模块保留 parent 版本字段，但使用 ${revision}；根 revision 单点定义，Flatten 适配安装发布，详见 TECH-027。 |
...
| TECH-021 | 内部数据对象和跨层入参必须有稳定且可验证的边界。 | `XXPO` 仅在 infrastructure 映射数据库；`XXDO` 仅在 `ddd-model` 表示无行为内部数据；DomainService、OutAdaptor 公开返回 `Result<XXDO>`。跨 Application、DomainService、OutAdaptor 的任意入参一律为 `XXCommand` 或 `XXParam`，禁止裸基本类型；Application assembler 将 Command 组装为 Domain Param，OutAdaptor 无额外语义时直接接收 Application Command，再由 converter 转为 adaptor 私有第三方请求。 |
| TECH-022 | Application 需要独立承担对象转换防腐职责。 | `ddd-application` 新增 `assembler`；Application Service 只编排“组装领域参数 → 调用 → 判断 Result → 返回”，不得直接 new Domain Param 或手写 Aggregate/DO 到 Application Result 的字段转换。不得为无额外语义的 OutAdaptor 调用复制 Command 为 Param。 |
| TECH-023 | Repository 不复用服务层 Param，分页不得泄漏 ORM 类型。 | `findById(String id)`；按 IDs 查询/删除接收标识集合，新增/修改接收 Aggregate。分页接收并返回 domain 自有分页模型，由 infrastructure 转换 MyBatis-Plus Page 和 PO。普通查询由 Application 调仓储，领域过滤由 DomainService 编排；筛选应先于分页，保证总数语义一致。当前无分页用例，不新增占位实现。 |
| TECH-024 | 四类公开入口签名与命名明确。 | Controller 使用 xxRequest/Result<xxResponse>，Application 使用 xxCommand/Result<xxResult>，DomainService 使用 param/Result<xxDO>，OutAdaptor 使用 Application xxCommand/Result<xxDO>。参数名按具体类型 lowerCamelCase；方法表达业务动作，保留协议与 Repository 例外；转换责任按已沉淀的 Java DDD 接口命名规范执行。 |
...
- `ddd-model`：无行为的项目内部 `XXDO`，仅承接领域服务和 output adaptor 的成功输出。
- `ddd-domain`：聚合、实体、值对象、参数、领域服务、仓储端口和领域输出 DO。
...
ddd-application/src/main/java/com/ddd/application/ddd/{assembler,command,param,result,service,adaptor}
...
| 域内读 | `GET /api/ddd/{id}` | input → `DddReadCommand` → Application 提取 ID → `DddRepository.findById(String)` | repository 直接返回聚合根，由 application assembler 转为结果；不创建查询领域 Param。 |
...
| 外部读 | `GET /api/ddd/{id}/external` | input → `DddExternalReadCommand` → `DddExternalReadApplication` → `DddOutputAdaptor` → converter → 第三方请求/响应 → `DddExternalReadDO` | application 定义端口，第三方模型不得进入 application/domain，DO 不进入 Controller。 |
...
| DATA-006 | `DddWriteDO`、`DddCalculateDO`、`DddRuleCalculateDO`、`DddExternalReadDO` | 无行为项目内部数据对象；领域服务和 OutAdaptor 的成功结果承载类型，由 Application 转为自身 Result。 |
...
```java
Result<DddWriteResponse> write(DddWriteRequest dddWriteRequest) {
    DddWriteCommand dddWriteCommand = inputAssembler.toCommand(dddWriteRequest);
    Result<DddWriteResult> applicationResult = dddWriteApplication.write(dddWriteCommand);
...
Result<DddWriteResult> write(DddWriteCommand dddWriteCommand) {
    DddWriteParam param = applicationAssembler.toDomainParam(dddWriteCommand);
    ...
    DddWriteResult result = applicationAssembler.toResult(dddWriteCommand, dataObject);
...
Result<DddWriteDO> write(DddWriteParam param) {
  try {
    ...
    String aggregateId = input.id().value();
    ...
    if (existing != null) return Result.success(toDuplicateDO(existing, aggregate.currentValue()));
    ...
    DddOperationEntity confirmed = aggregate.confirm(pending, calculated, Instant.now());
    if (!Boolean.TRUE.equals(dddRepository.save(aggregate))) throw domainConflict();
    return Result.success(toWriteDO(confirmed, aggregate.currentValue(), rule.reason()));
...
DddAggregate findById(String id) {
    ...
    if (stored == null) return DddAggregate.of(DddEntity.open(aggregateId));
    return DddAggregate.of(DddEntity.restore(aggregateId, new DddValue(stored.getCurrentValue()),
...
```java
Result<DddReadResult> query(DddReadCommand dddReadCommand) {
    try {
        String id = dddReadCommand.id();
        DddAggregate aggregate = dddRepository.findById(id);
        DddReadResult result = applicationAssembler.toResult(aggregate);
        return Result.success(result);
    } catch (DomainException exception) {
        return Result.failure(exception.errorCode());
    }
...
Result<DddRuleResult> calculate(DddRuleCommand dddRuleCommand) {
    DddRuleParam param = applicationAssembler.toDomainParam(dddRuleCommand);
    Result<DddRuleCalculateDO> domainResult = dddRuleDomainService.calculate(param);
    return toApplicationResult(domainResult);
}
...
Result<DddCalculateResult> calculate(DddCalculateCommand dddCalculateCommand) {
    DddCalculateParam param = applicationAssembler.toDomainParam(dddCalculateCommand);
    Result<DddCalculateDO> domainResult = calculateDomainService.calculate(param);
    return toApplicationResult(domainResult);
}
```

域内读由 application assembler 转换聚合内实体；纯计算不访问 repository 或 aggregate。

外部读：

```java
Result<DddExternalResult> query(DddExternalReadCommand dddExternalReadCommand) {
    Result<DddExternalReadDO> adaptorResult = outputAdaptor.query(dddExternalReadCommand);
    if (!adaptorResult.success()) {
        return Result.failure(adaptorResult.code(), adaptorResult.message());
    }
    DddExternalReadDO dataObject = adaptorResult.data();
    DddExternalResult result = applicationAssembler.toResult(dataObject);
    return Result.success(result);
}

Result<DddExternalReadDO> query(DddExternalReadCommand dddExternalReadCommand) {
    try {
        DddExternalRequest request = converter.toExternalRequest(dddExternalReadCommand);
        DddExternalResponse response = invokeRemote(request);
        DddExternalReadDO dataObject = converter.toDataObject(response);
...
| 2.5 | 2026-09-16 | 正式区分 `XXPO`、`XXDO`、`XXResult` 与外部协议对象；DomainService、OutAdaptor 返回 `Result<XXDO>`；Controller→Application、Application→OutAdaptor、Application→DomainService 均以 Command/Param 传入，外部请求由 output converter 从 Command 创建。 | 固化项目内模型与外部协议边界，杜绝裸基本类型跨服务边界和外部 DTO 泄漏。 |
| 2.6 | 2026-09-16 | 增加 `DddApplicationAssembler` 与 application `param`：统一 Command 到 Domain/OutAdaptor Param，以及 Aggregate/DO 到 Application Result 的转换。Application Service 仅保留用例编排。 | 防止 Application 层在每个用例中重复对象组装和字段映射，使 HTTP、应用、领域、外部协议的防腐职责可独立审查。 |
| 2.7 | 2026-09-16 | 删除无额外语义的 `DddExternalReadParam`；OutAdaptor 恢复直接接收 `DddExternalReadCommand`，converter 从 Command 创建第三方请求。Application assembler 仅负责领域参数和结果转换。 | 防止为形式一致而重复复制同一份数据，保持 OutAdaptor 的 Application 契约最小、可读。 |
| 2.8 | 2026-09-16 | 仓储 ID 查询恢复基础标识入参，删除 `DddReadParam` 及无意义查询映射；明确 Aggregate 写入、域内分页契约与 infrastructure 分页转换。 | 对象入参规则仅约束服务边界，不强加到基础 CRUD；保持仓储契约框架无关。 |
| 2.9 | 2026-09-16 | 修整四类公开入口参数名及业务方法名，同步调用和 Javadoc；将命名、转换与例外沉淀到 solo-delivery 的 Java DDD 按需规范。 | 明确模板签名，减少后续生成代码中的通用命名和跨层混用。 |

## 本轮规范收敛与编译门禁（TECH-025）

当前有效约束统一查阅 [19 Java DDD开发规范.md](<19 Java DDD开发规范.md>)；此前版本和决策记录仅保留演进证据，发生冲突以该规范的最终规则及覆盖关系为准。

根 POM 将 Checkstyle 绑定到 `validate`，子模块继承；`compile/test/package` 均先经过门禁。检查配置随工程保存，不依赖外部图片或在线规范。对公开方法注释、命名后缀、简单四层入口签名、参数命名、行宽及基础格式等执行自动检查；完整依赖图、领域内聚、错误处理行为、转换正确性、POM 版本治理、禁止 SQL 的全部形式等仍须人工审查或补充工具，详见规范第 10 节。

规则聚合改为仅持有 `DddRuleEntity`，计算及状态校验委托实体，保留既有调用方式。Skill 提供可复用 XML 和幂等安装脚本，在未来采用本规范的 Java DDD 聚合工程开发阶段安装。发生既有配置冲突时停止覆盖，先融合规则，不破坏用户配置。

| 2.10 | 2026-09-16 | 按 SRC-035 汇总最终规范，接入 Maven validate 门禁，补齐公开方法/字段 Javadoc，并收敛规则聚合为实体委托；新项目自动安装同源资产。 | 形成可复用且明确检查边界的代码生成规范。 |
## start 显式装配清单（TECH-026 / SRC-037）

`ddd-start/pom.xml` 直接声明 common、client、model、domain、application、infrastructure、adaptor 七个运行模块，补齐原清单中的 common 和 model。start 仍仅承载启动和配置，不因此获得业务职责；业务模块不得反向依赖 start。

真实项目按照本服务实际运行边界裁剪模块；测试专用依赖使用 test scope，依赖版本保持根 POM 集中管理。该约定是本模板的工程选择，不是所有 DDD 项目的普遍标准。采用 POM、有效依赖树及构建/启动路径验证，不宣称 Checkstyle 检查该规则。solo 公共规范 MOD-009、阶段指引和代码快照同步更新。
## 项目版本单点管理与发布适配（TECH-027 / SRC-038）

根 POM 使用 `<version>${revision}</version>`，在 properties 中唯一声明默认 `<revision>0.0.1-SNAPSHOT</revision>`；八个子模块仅将 parent.version 改为 `${revision}`，不声明独立项目版本或重复 revision。内部模块依赖仍由根以 `${project.version}` 管理；外部 Spring Boot parent 版本保持根声明。后续改根 revision 或传 `-Drevision=0.0.2-SNAPSHOT` 即可统一升级。

根 build/plugins 配置 Flatten 1.8.0：updatePomFile=true、resolveCiFriendliesOnly，flatten 绑定 process-resources，clean 绑定 clean。生成 `.flattened-pom.xml` 供 install/deploy 使用，不改写源码 POM；该模式只解析 CI 版本变量，保留其他 POM 结构。生成文件被 Git 忽略，不纳入 Skill 快照。版本配置与发布能力通过默认构建、临时覆盖版本安装、独立消费者及 clean 清理验证，不由 Checkstyle 自动证明。本轮不执行远程发布。

solo 规范 MAV-001/005/006、阶段指引、参考快照 v1.2 与便携包同步，当前项目的 `19 Java DDD开发规范.md` 保持公共源一致。
