...
Result<DddWriteResult> execute(DddWriteCommand command) {
    DddWriteParam param = applicationAssembler.toDomainParam(command);
    ...
    return Result.success(applicationAssembler.toResult(command, domainResult.data()));
}

Result<DddWriteDO> execute(DddWriteParam param) {
  try {
...
```java
Result<DddReadResult> query(DddReadCommand command) {
    try {
        String id = command.id();
        DddAggregate aggregate = dddRepository.findById(id);
        return Result.success(applicationAssembler.toResult(aggregate));
    } catch (DomainException exception) {
        return Result.failure(exception.errorCode());
    }
}
...
Result<DddRuleResult> execute(DddRuleCommand command) {
    DddRuleParam param = applicationAssembler.toDomainParam(command);
...
Result<DddCalculateResult> execute(DddCalculateCommand command) {
    DddCalculateParam param = applicationAssembler.toDomainParam(command);
...
```java
Result<DddExternalResult> query(DddExternalReadCommand command) {
    Result<DddExternalReadDO> adaptorResult = outputAdaptor.query(command);
    return Result.success(applicationAssembler.toResult(adaptorResult.data()));
}

Result<DddExternalReadDO> query(DddExternalReadCommand command) {
    try {
        DddExternalRequest request = converter.toExternalRequest(command);
